...
public class LogUtility {
	...
	public static void logInfo(String methodName, String debugOption, Class<?> clazz, String message) {
		...
		Activator.getDefault().log(new Status(IStatus.INFO, Activator.ID, IStatus.INFO, clazz.getName() + ":" //$NON-NLS-1$
	...
	public static void logWarning(String methodName, String debugOption, Class<?> clazz, String message) {
		...
		Activator.getDefault().log(new Status(IStatus.WARNING, Activator.ID, IStatus.WARNING, clazz.getName() + ":" //$NON-NLS-1$
				+ ((methodName == null) ? "<unknown>" //$NON-NLS-1$
						: methodName)
				+ ":" //$NON-NLS-1$
				+ ((message == null) ? "<empty>" : message), //$NON-NLS-1$
	...
	public static void logError(String methodName, String debugOption, Class<?> clazz, String message, Throwable t) {
		...
		Activator.getDefault().log(new Status(IStatus.ERROR, Activator.ID, IStatus.ERROR, clazz.getName() + ":" //$NON-NLS-1$
				+ ((methodName == null) ? "<unknown>" //$NON-NLS-1$
						: methodName)
				+ ":" //$NON-NLS-1$
	...
	public static void trace(String methodName, String debugOptions, Class<?> clazz, String message) {
		Trace.trace(Activator.ID, debugOptions, clazz, methodName, message);
	}

	public static void traceException(String methodName, String debugOption, Class<?> clazz, String message,
			Throwable t) {
		Trace.catching(Activator.ID, debugOption, clazz, ((methodName == null) ? "<unknown>" : methodName) + ":" //$NON-NLS-1$ //$NON-NLS-2$
